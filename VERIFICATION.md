...
- GitHub's active account changed from the earlier verified Harshyadav442277 account to the already-signed-in 0xsaroj001 account. An unqualified create command therefore created the private backup under that account. The destination mistake was disclosed and corrected: the transfer completed, and the API now reports Harshyadav442277/speculate-regression-room with isPrivate:true. At the user's instruction the active CLI account was switched to Harshyadav442277 and origin updated. The previous account's automatically retained write access was removed; the collaborators API lists only Harshyadav442277 as admin. Source remained private throughout the checked states.
